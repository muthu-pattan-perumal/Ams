        ...
        <div className="animate-in fade-in duration-500">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
                <div>
                    ...
                    <p className="text-slate-500">Manage vendors and inventory accounts</p>
                </div>
                <button onClick={() => { setEditingSupplier(null); setFormData({ name: '', phone: '', address: '', email: '', openingBalance: 0 }); setModalOpen(true); }} className="btn-primary w-full sm:w-auto">
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50/50 border-b border-slate-100/80">
                                <th className="px-6 py-4 font-bold text-slate-600 text-xs lg:text-sm uppercase tracking-wider">Supplier</th>
                            ...
                            {suppliers.map(s => (
                                <tr key={s.id} className="hover:bg-slate-50 transition-all group">
                                    <td className="px-6 py-5">
                                        <div className="flex items-center gap-3">
                                            <div className="w-10 h-10 rounded-xl bg-purple-500/10 flex items-center justify-center text-purple-600 shrink-0">
                                                <i className="mdi mdi-truck text-xl"></i>
                                            </div>
                                            <div className="min-w-0">
                                                <button onClick={() => navigate(`/ledger/supplier/${s.id}`)} className="font-bold text-slate-900 truncate hover:text-primary-9 hover:underline transition-all block">
                                                    {s.name}
                                                </button>
                                    ...
                                    <td className="px-6 py-5 text-right">
                                        <p className={`font-black tracking-tight text-lg ${(s.balance + s.openingBalance) >= 0 ? 'text-slate-900' : 'text-rose-600'}`}>
                                            ${Math.abs(s.balance + s.openingBalance).toFixed(2)}
                                            <span className="ml-1 text-[10px] uppercase font-bold opacity-50">
                                                {(s.balance + s.openingBalance) >= 0 ? 'Dr' : 'Cr'}
                                            </span>
                                        </p>
                                    </td>
                                    <td className="px-6 py-5 text-right">
                                        <div className="flex items-center justify-end gap-1">
                                            <button onClick={() => navigate(`/ledger/supplier/${s.id}`)} className="p-2 text-slate-400 hover:text-primary-9 hover:bg-primary-9/10 rounded-lg transition-all" title="View Ledger">
                                                <i className="mdi mdi-book-open-variant text-xl"></i>
                                            </button>
                                            <button onClick={() => { setEditingSupplier(s); setFormData(s); setModalOpen(true); }} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-600/10 rounded-lg transition-all" title="Edit">
                                                <i className="mdi mdi-pencil text-xl"></i>
                                            </button>
                                            <button onClick={async () => { if (confirm('Delete supplier?')) { await api.delete(`/suppliers/${s.id}`); fetchSuppliers(); } }} className="p-2 text-slate-400 hover:text-error-main hover:bg-error-main/10 rounded-lg transition-all" title="Delete">
            ...
            <div className="md:hidden space-y-4 mb-8">
                {suppliers.map(s => (
                    <div key={s.id} className="card p-5 border-slate-100 shadow-sm relative overflow-hidden">
                        <div className={`absolute top-0 left-0 w-1 h-full ${(s.balance + s.openingBalance) >= 0 ? 'bg-purple-600' : 'bg-rose-500'}`}></div>
                        <div className="flex justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <div className="min-w-0">
                                    <h3 className="font-bold text-slate-900 truncate leading-tight">{s.name}</h3>
                                    <p className="text-xs text-slate-500">{s.phone}</p>
                                </div>
                            </div>
                            <div className="text-right">
                                <p className={`font-black text-lg leading-tight ${(s.balance + s.openingBalance) >= 0 ? 'text-slate-900' : 'text-rose-600'}`}>
                                    ${Math.abs((Number(s.balance) || 0) + (Number(s.openingBalance) || 0)).toFixed(2)}
                                </p>
                        ...
                        <div className="grid grid-cols-3 gap-2 border-t border-slate-50 pt-4">
                            <button onClick={() => navigate(`/ledger/supplier/${s.id}`)} className="flex flex-col items-center gap-1 py-1 text-slate-500 hover:text-primary-9 transition-all">
                                <i className="mdi mdi-book-open-variant text-xl"></i>
                                <span className="text-[10px] font-bold uppercase">Ledger</span>
                            </button>
                            <button onClick={() => { setEditingSupplier(s); setFormData(s); setModalOpen(true); }} className="flex flex-col items-center gap-1 py-1 text-slate-500 hover:text-indigo-600 transition-all">
                                <i className="mdi mdi-pencil text-xl"></i>
                                <span className="text-[10px] font-bold uppercase">Edit</span>
                            </button>
                            <button onClick={async () => { if (confirm('Delete?')) { await api.delete(`/suppliers/${s.id}`); fetchSuppliers(); } }} className="flex flex-col items-center gap-1 py-1 text-slate-500 hover:text-error-main transition-all">
                                <i className="mdi mdi-delete text-xl"></i>
